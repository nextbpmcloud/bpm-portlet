...
<xsl:stylesheet version="1.0"
   ...
   <xsl:template match="/HTML/HEAD">
      ...
      <xsl:apply-templates select="SCRIPT[not(contains(concat(' ',normalize-space(@class),' '),' noProxy '))]"/>
   ...
   <!-- Rewrite links -->
   <xsl:template match="A[not(contains(concat(' ',normalize-space(@class),' '),' noProxy '))]/@href">
   ...
   <xsl:template match="SCRIPT[not(contains(concat(' ',normalize-space(@class),' '),' noProxy '))]/@src">
      <xsl:attribute name="src">
         <xsl:value-of select="bridge:link($bridge, $base, .)"/>
      </xsl:attribute>
   </xsl:template>

   <!-- Rewrite iframe source -->
   <xsl:template match="IFRAME[not(contains(concat(' ',normalize-space(@class),' '),' noProxy '))]/@src">
      <xsl:attribute name="src">
         <xsl:value-of select="bridge:link($bridge, $base, .)"/>
      </xsl:attribute>
   </xsl:template>
   ...
   <xsl:template match="/HTML/HEAD/LINK[not(contains(concat(' ',normalize-space(@class),' '),' noProxy '))]">
      <STYLE type="text/css">@import "<xsl:value-of select="bridge:link($bridge, $base, @href)"/>";</STYLE>
   </xsl:template>

   <xsl:template match="FORM[not(contains(concat(' ',normalize-space(@class),' '),' noProxy '))]/@action">
